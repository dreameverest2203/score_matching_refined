...
def itemwise_f(params, state, x, std):
    ...
    return f.apply(params, state, x.reshape(1, -1), std.reshape(1, -1), conf.sigma, is_training=False)


@partial(jax.jit, static_argnames=("n_iterations", "n_burn_in"))
def annealed_langevin(params, state, key, x_0, std_array, epsilon, n_iterations, n_burn_in):
    def langevin(carry, std_array):
        ...
        def inner_fn(carry, key):
            ...
            x = x + epsilon * out.squeeze() + jnp.sqrt(2 * epsilon) * random.normal(key, shape=x.shape) 
            return (x,state), x
        (x,state), xs = jax.lax.scan(inner_fn, (x_0,state), random.split(key, n_iterations))
