...
data_loader = DataLoader(
    dataset, batch_size=conf.batch_size, shuffle=True, num_workers=2
)
...
# # # Model Training
# train_loss, params, state = train_wrapper(data_loader, params, state, conf.num_epochs)
# print("Model training done\n")

# with open(f"model_weights/dae_model_{conf.num_samples}samples.pickle", "wb") as handle:
...
init_x = rnd.normal(
    rnd.PRNGKey(4),
    (1, 28, 28, conf.num_samples),
)

out = ode_sampler(params, state, init_x)
...
save_image(grid_img, fp=f"images/MNIST_{conf.num_samples}.png", nrow=len(indices))
